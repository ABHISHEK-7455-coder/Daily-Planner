...
//   );
// }
// pages/Login.jsx — Convex version
// Your EXACT Login.jsx file — only 3 things changed:
//   1. Remove: import { firebaseAuth } from "../Firebase"
//   2. Add:    import { useAuthActions } from "@convex-dev/auth/react"
//   3. Replace the 3 firebaseAuth calls + handleGoogleLogin body

import React, { useState, useEffect } from "react";
import { useNavigate } from "react-router-dom";
import "./Login.css";
import { useAuthActions } from "@convex-dev/auth/react";   // ← CHANGED (was firebaseAuth)
import { useAuth } from "../Context/Authcontext";

export default function Login() {
  const navigate = useNavigate();
  const { user } = useAuth();
  const { signIn } = useAuthActions();                     // ← NEW
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === "login") {
        // ← CHANGED: was firebaseAuth.signInWithPassword({ email, password })
        await signIn("password", { email, password, flow: "signIn" });
        navigate("/");

      } else if (mode === "signup") {
        // ← CHANGED: was firebaseAuth.signUp({ email, password, options: { data: { full_name: name } } })
        await signIn("password", { email, password, name, flow: "signUp" });
        setSuccess("Account created! Taking you in…");
        setTimeout(() => navigate("/"), 800);

      } else if (mode === "forgot") {
        // ← CHANGED: was firebaseAuth.resetPasswordForEmail(email)
  ...
  const handleGoogleLogin = async () => {
    setError("");
    // ← CHANGED: was firebaseAuth.signInWithOAuth()
    try {
      await signIn("google");
    } catch (err) {
